...
class GradientDescentSolver(Solver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        feed_dict = dict(zip([model.input_ph, target_ph], [train_data[0], train_data[1]]))
        "*** YOUR CODE HERE ***"
        ...
        for iter_ in range(self.iterations):
            ...
            session.run(update_ops, feed_dict=feed_dict) 
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in xrange(self.iterations):
            ...
            t_p_h = trainGenerator.next()
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]:t_p_h[0], placeholders[1]:t_p_h[1]})
            session.run(update_ops, feed_dict={placeholders[0]:t_p_h[0], placeholders[1]:t_p_h[1]})
            v_p_h = valGenerator.next()
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: v_p_h[0], placeholders[1]: v_p_h[1]})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        valGenerator   = MinibatchIndefinitelyGenerator( val_data  , self.batch_size, self.shuffle )
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            #util.raiseNotDefined()
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
